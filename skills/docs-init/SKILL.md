...
Goal: drop the Project Docs app into the **current working directory** (the project root), wire it up, and tell the user how to run it. The app and its config live together in a single hidden `.project-docs/` folder so they don't clutter the project. Be idempotent — if it's already installed, just re-run setup and report status.

## Steps

1. **Check if it already exists.** If a `.project-docs/app/` folder is already present, skip to step 3.

2. **Fetch the app into `.project-docs/app/`.** Use whichever source applies:
   - If the env var `PROJECT_DOCS_SRC` is set, copy from that local path:
     `mkdir -p .project-docs && cp -R "$PROJECT_DOCS_SRC" .project-docs/app`
   - Otherwise clone the published repo:
     `npx degit ledminh/project-docs .project-docs/app`
...
5. **Tell the user how to run it:**
   ```bash
   cd .project-docs/app && npm start    # → http://localhost:4500
   ```
...
## Notes
- `.project-docs/` is the installed app (`.project-docs/app/`) and is entirely gitignored. The contract lives in `.claude/project-docs.md` (tracked) so no folder mixes tracked + ignored content.
