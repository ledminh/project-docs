...
2. **Fetch the app into `project-docs/`.** Use whichever source applies:
   ...
   - Otherwise clone the template repo (replace `<OWNER>` with the published repo):
     `npx degit <OWNER>/project-docs project-docs`
...
## Notes
- The app is files-first: everything lives in the project's `docs/` and `tasks/` folders, not in a database. `.project-docs/state.db` holds only todos + config.
